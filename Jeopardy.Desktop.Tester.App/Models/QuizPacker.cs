...
namespace Jeopardy.Desktop.Tester.App.Models
{
    public static class QuizPacker
    {

        public static ValidationResult Validate(Quiz quiz)
        {
            ...
            for (int i = 0; i < quiz.Rounds.Count; i++)
            {
                ...
                for (int j = 0; j < quiz.Rounds[i].Categories.Count; j++)
                {
                    ...
                    for (int k = 0; k < quiz.Rounds[i].Categories[j].Questions.Count; k++)
                    {
        ...
        public static ValidationResult Pack(Quiz quiz)
        {
        ...
        public static void Unpack(Quiz quiz)
        {
            foreach (QuizRound? round in quiz.Rounds)
            {
                foreach (QuizCategory? category in round.Categories)
        ...
        private static void PackTextQuestion(Question question)
        {
            question.RawContent = Encoding.UTF8.GetBytes(question.ContentPath);
        }
        ...
        {
            string rnd = $"R{round}:";
            string cat = category == -1 ? "" : $"C{category}:";
            string qst = question == -1 ? "" : $"Q{question}:";
